<div class="snaps index">
	<h2><?php echo __('Snaps'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('name'); ?></th>
			<th><?php echo $this->Paginator->sort('university'); ?></th>
			<th><?php echo $this->Paginator->sort('undergraduate'); ?></th>
			<th><?php echo $this->Paginator->sort('schoolyear'); ?></th>
			<th><?php echo $this->Paginator->sort('love'); ?></th>
			<th><?php echo $this->Paginator->sort('questionfree'); ?></th>
			<th><?php echo $this->Paginator->sort('questionanswer'); ?></th>
			<th><?php echo $this->Paginator->sort('other'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($snaps as $snap): ?>
	<tr>
		<td><?php echo h($snap['Snap']['id']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['name']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['university']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['undergraduate']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['schoolyear']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['love']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['questionfree']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['questionanswer']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['other']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['created']); ?>&nbsp;</td>
		<td><?php echo h($snap['Snap']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $snap['Snap']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $snap['Snap']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $snap['Snap']['id']), array(), __('Are you sure you want to delete # %s?', $snap['Snap']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Snap'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Snapphotos'), array('controller' => 'snapphotos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Snapphoto'), array('controller' => 'snapphotos', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Snaptags'), array('controller' => 'snaptags', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Snaptag'), array('controller' => 'snaptags', 'action' => 'add')); ?> </li>
	</ul>
</div>
